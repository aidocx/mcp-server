...
const inputSchema = {
    query: z.string().optional().describe("Search query text (max 500 chars). Searches document names and content."),
...
export function register(server: McpServer, client: AiDocxClient) {
    ...
    server.registerTool(
        ...
        {
            description:
                "Search documents in AiDocX. Searches across all document types " +
